...
$regions        = $regionIni->hasVariable( 'Regions', 'LocaleCountryList' ) ? $regionIni->variable( 'Regions', 'LocaleCountryList' ) : array();
$defaultCountry = $xini->hasVariable( 'ShopAccountHandlerDefaults', 'DefaultCountryCode' ) ? array( $xini->variable( 'ShopAccountHandlerDefaults', 'DefaultCountryCode' ) ) : false;
if( $defaultCountry && in_array( 'USA', $defaultCountry ) ) {
    $defaultCountry[] = 'CAN';
}
$enableRestircted = $xini->hasVariable( 'Settings', 'EnableRestircted' ) ? in_array( $xini->variable( 'Settings', 'EnableRestircted' ), array( 'enabled', 'yes', 'true' ) ) : false;
...
if( $enableRestircted ) {
    foreach( $siteIni->variable( 'RegionalSettings', 'LanguageSA' ) as $key => $value ) {
        if(
            isset( $regions[$key] ) && strlen( $regions[$key] ) > 0
        ) {
            $restirctedCountries = array_merge(
                $restirctedCountries, explode( ';', $regions[$key] )
            );
        } else {
            $country = explode( '-', $key );
            $country = $country[1];
    ...
    foreach( $restirctedCountries as $key => $alpha2 ) {
        ...
        if(
            isset( $allowedStates[$alpha2] ) || in_array( $country['Alpha3'], $additionalCountries )
        ) {
...
if( $module->isCurrentAction( 'Store' ) ) {
    ...
    if( $fields['country']['enabled'] == true ) {
        ...
        if( $country == '' and $fields['country']['required'] == true ) {
            ...
            if( $xini->hasVariable( 'Settings', 'CountryWihtStatesList' ) and in_array( $country, $xini->variable( 'Settings', 'CountryWihtStatesList' ) )
                and $state == '' ) {
                $inputIsValid                   = false;
                $fields['country']['errors'][0] = ezpI18n::tr( 'extension/xrowecommerce', 'No billing country has been selected.' );
            } elseif(
                ( $defaultCountry && !in_array( $country, $defaultCountry ) ) || ( count( $restirctedCountries ) > 0 && in_array( $country, $restirctedCountries ) )
            ) {
                ...
                if( $http->hasPostVariable( 'shipping' ) ) {
                    ...
                    $fields['country']['errors'][0] = ezpI18n::tr(
                            'extension/xrowecommerce', 'Sorry, it\'s not possible to ship to the country you\'ve selected from this site. Other regions may be selected from the menu at the top of the page.'
                    );
                }
            }
        }
                ...
                if( isset( $excludeStates[$country] ) ) {
                    ...
                    if( in_array( $state, $excludeStateCodes ) ) {
                        ...
                        $fields['state']['errors'][0] = ezpI18n::tr(
                                'extension/xrowecommerce', 'Sorry, it\'s not possible to ship to the state you\'ve selected from this site. Other regions may be selected from the menu at the top of the page.'
                        );
                    }
                }
    ...
    if( $http->hasPostVariable( 'message' ) && strlen( strip_tags( $http->postVariable( 'message' ) ) ) > $maxCommentSize ) {
        ...
        $fields['message']['errors'][0] = ezpI18n::tr(
                'extension/xrowecommerce', 'Comment is too long (it should not be longer than %max_size characters)', null, array( '%max_size' => $maxCommentSize )
        );
    }
    ...
    } elseif( $shipping == '1' and ( $shippingtype == "usps_international" or $shippingtype == "usps_international_guaranteed" )
        and $country == "USA" ) {
        $inputIsValid                        = false;
    ...
    } elseif( $shipping == '1' and ( $shippingtype == "ups_ground" or $shippingtype == "ups_air_2ndday" or $shippingtype
        == "ups_air_nextday" ) and $country != "USA" ) {
        $fields['shippingtype']['errors'][0] = ezpI18n::tr( 'extension/xrowecommerce', 'Please select a proper shipping method for your destination.' );
    } elseif( $shipping == '0' and ( $shippingtype == "usps_international" or $shippingtype == "usps_international_guaranteed" )
        and $s_country == "USA" ) {
        $inputIsValid                        = false;
        $fields['shippingtype']['errors'][0] = ezpI18n::tr( 'extension/xrowecommerce', 'Please select a proper shipping method for your destination.' );
    } elseif( $shipping == '0' and ( $shippingtype == "ups_ground" or $shippingtype == "ups_air_2ndday" or $shippingtype
        == "ups_air_nextday" ) and $s_country != "USA" ) {
        $fields['shippingtype']['errors'][0] = ezpI18n::tr( 'extension/xrowecommerce', 'Please select a proper shipping method for your destination.' );
    }
        ...
        if( $fields['s_country']['enabled'] == true ) {
            ...
            if( $s_country == '' and $fields['s_country']['required'] == true ) {
                ...
                if( in_array( $s_country, $xini->variable( 'Settings', 'CountryWithStatesList' ) ) and $s_state == '' ) {
                    ...
                    ( $defaultCountry && !in_array( $s_country, $defaultCountry ) ) || ( count( $restirctedCountries ) > 0
                    && in_array( $s_country, $restirctedCountries ) )
                ) {
                    $inputIsValid                   = false;
                    $fields['country']['errors'][0] = ezpI18n::tr(
                            'extension/xrowecommerce', 'Sorry, it\'s not possible to ship to the country you\'ve selected from this site. Other regions may be selected from the menu at the top of the page.'
    ...
    if( class_exists( 'xrowVerification' ) and $fields_captcha['enabled'] == 'true' and $accessAllowed['accessWord'] != 'yes'
        and isset( $_SESSION['xrowCaptchaSolved'] ) && empty( $_SESSION['xrowCaptchaSolved'] ) ) {
        $recaptcha    = true;
        ...
        foreach( $basketPrproducts as $basketPrproduct ) {
            ...
            if( in_array( $productNumber, $restrictedProducts ) ) {
                ...
                $restrictedProductErrors[] = ezpI18n::tr(
                        'extension/xrowecommerce', $restrictedProductMessage, null, array(
    ...
    if( $inputIsValid ) {
        ...
        if( $shipping != '1' ) {
            ...
            /* Shipping address */
        } /* Shippingaddress is equal or not */ else {
